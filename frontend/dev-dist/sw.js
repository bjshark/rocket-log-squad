...
define(['./workbox-37d4c263'], (function (workbox) { 'use strict';
  ...
  workbox.registerRoute(({
    url,
    request
  }) => request.method === "GET" && /\/api\/v1\/(rockets|engines)(\/|$)/.test(url.pathname), new workbox.StaleWhileRevalidate({
  ...
  workbox.registerRoute(({
    url,
    request
  }) => request.method === "GET" && /\/api\/v1\/(my|weather)(\/|$)/.test(url.pathname), new workbox.NetworkFirst({
